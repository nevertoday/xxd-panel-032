<p align="center">
  <img src="./assets/banner.svg" alt="XXD Panel 032 project banner" width="1200">
</p>
...
[![Four Modes](https://img.shields.io/badge/Modes-4-d75d32?style=flat-square)](#four-outputs-one-integrated-wordmark-system)
...
032 reverses that logic:

```text
lock identity / proportion / gesture / function → resolve one short native main text → extract the strongest contour, opening, axis, connection, or negative shape → embed it into an actual glyph, counter, terminal, crossbar, junction, ligature, or whole-word silhouette → redraw the remaining letters with shared stroke DNA → refine kerning, counters, and optical centre → prove thumbnail and monochrome reading → present one medium-scale mark with minimal colour and generous whitespace
```
...
- **One source intervention:** embed one defining contour, function, opening, connection, or negative shape into real letterform construction.
- **Two-way dependency:** removing the intervention makes the wordmark generic; removing the lettering does not leave a standalone icon.
- **Shared lettering DNA:** stroke weight, terminals, curves, angles, counters, baseline, and optical corrections belong to one authored system.
...
The repository reserves [`assets/examples/`](assets/examples/) for future work. Only finished 032 artwork verified by the project owner will be added; no post or image from another style is used as a placeholder.

Future samples will demonstrate 032's adaptability. Their subjects, whitespace ratios, palette, copy, and canvas ratios will never become generation references or defaults.

## Four outputs, one integrated wordmark system
...
| --- | --- | --- |
| `top-bottom` | source-adaptive | complete source above, 032 source-integrated custom wordmark below; both panels retain the source size and split exactly 50/50 |
| `left-right` | source-adaptive | complete source left, 032 source-integrated custom wordmark right; both panels retain the source size and split exactly 50/50 |
...
Exact user pixels > explicit ratio or destination > source adaptation for ordinary modes. The original `032.md` used a 3:4 creative canvas, but that historical example is not a silent default in the current Skill.
...
- **Independent set:** every device references only the source photograph and may explore a different mark scale, source-feature intervention, silhouette, spacing rhythm, monochrome/accent relation, and whitespace balance while preserving the locked main text.
...
Automatic copy uses a verified proper name when available; otherwise it derives one short, memorable word or phrase from source identity, theme, place clue, action state, or supported meaning. It never invents an organisation or ownership.

The default is one custom wordmark. Add zero to two much smaller descriptors only when they carry real information. Design natively for Chinese, Japanese, Korean, Arabic, or Latin anatomy; preserve exact user wording verbatim and never render pseudo-lettering. Copy and graphic must pass the two-way dependency and unrelated-image swap tests.
...
The image model creates the integrated source-to-letter intervention, native custom wordmark, shared stroke and counter system, optical kerning, monochrome/accent relation, refined silhouette, and generous whitespace. `scripts/compose_panel.py` only plans canvases, performs exact 50/50 raster composition, finalises dimensions, and audits results. It never fakes artwork with programmatic drawing.
...
- [Chinese full prompt](references/xxd-panel-032-prompt.zh-CN.md)
- [English full prompt](references/xxd-panel-032-prompt.en.md)
- [Original style brief](references/032-source.md)
...
**The image does not sit beside the name; it becomes the name.**
